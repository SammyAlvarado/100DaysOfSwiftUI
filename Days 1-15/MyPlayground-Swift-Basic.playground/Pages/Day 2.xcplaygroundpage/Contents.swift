...
func findHighRisk() -> [String] {
    var result: [String] = []
    for agent in agents {
        if agent.accessLevel >= 8 {
            result.append("\(agent.realName), level: \(agent.accessLevel)")
            
            if agent.compromised == true {
                result.append("**WARNING** **COMPROMISED**")
            }
        }
    }
...
func findAgentLevel() -> String {
    ...
    return "\(lowCount) # low level agents, \(midCount) # mid level agents, and \(highCount) # high level agents"
}
...
func printAgentCoverName() -> [String] {
    var result: [String] = []
    for agent in agents {
        result.append("Cover Name: \(agent.coverName), Access Level: \(agent.accessLevel)")
    }
